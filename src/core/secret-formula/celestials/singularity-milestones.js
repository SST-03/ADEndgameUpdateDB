...
export const singularityMilestones = {
  ...
  perPurchaseDimMult: {
    ...
    repeat: new Decimal(0),
    limit: 1,
    description: "Multiply the Dimension Per-Purchase Multiplier for the other four Dimension types based on Singularities",
  ...
  darkFromGalacticPower: {
    ...
    effect: () => 1 + Math.pow(Decimal.log10(Currency.galacticPower.add(1)), 0.2),
    effectFormat: x => formatX(x, 2, 2),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_LAITELA,
  },
  antimatterExponentPower: {
    ...
    effect: () => 1 + (Math.pow(Math.log10(Decimal.log10(Currency.singularities.add(1)) + 1) / 2, 3) / 100),
    effectFormat: x => (x === 1 ? "Unlocked" : "Locked"),
    upgradeDirection: LAITELA_UPGRADE_DIRECTION.BOOSTS_MAIN,
  }
};
